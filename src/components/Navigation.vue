...
function initResultStep() {
	set_result.value = calculator(result_obj.value.city, result_obj.value.rent_type, result_obj.value.personal, result_obj.value.tours_per_month, result_obj.value.wanted_price_per_month);
...
<template>
	<div class="navigation">
		<div class="actions" v-if="show_result">
			<button class="coral-btn gost"
							@click="appReset()">Посчитать заново
			</button>
			<button class="coral-btn" :disabled="!type_of_result">Оставить заявку</button>
		</div>
		<div class="actions" v-else>
			<button class="coral-btn" @click="increaseStep()" v-if="counter < 2">Начать</button>
			<button class="coral-btn gost" @click="decreseStep()" v-else>Назад</button>
			<button class="coral-btn" @click="inputValue && increaseStep()"
							v-if="counter > 1 && counter < 6" :disabled="!inputValue">Дальше
			</button>
			<button class="coral-btn"
							@click="initResultStep()"
							v-if="counter === 6" :disabled="!inputValue">Посчитать
			</button>
		</div>
		<div class="progress">
			<div class="progress__bar">
				<div class="progress__fill" :style="{width: counter / 6 * 100 + '%'}"
						 :class="{success: type_of_result !== null && type_of_result, failed: type_of_result !== null && !type_of_result}"></div>
			</div>
			<span v-if="!show_result">шаг {{ counter }} из 6</span>
			<svg v-else-if="type_of_result" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16"
					 fill="none">
				<path
